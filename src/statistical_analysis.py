...
def compare_volume_corrected_nm(file_handler, sec_file_handler, acquisition1, acquisition2):
    ...
    cell_area_arhgdia = [idsc.get_cell_area(sec_file_handler, image) for image in arhgdia]
...
def compute_volume_corrected_nm(file_handler, image_list):
    ...
    #cell_volume= [idsc.get_cell_area(file_handler, image) for image in image_list]
...
def compare_nucleus_area(file_handler, sec_file_handler, acquisition1, acquisition2):
    ...
    #plt.boxplot([nucleus_area_arhgdia, nucleus_area_arhgdia_cultured])
    ...
    #plt.ylabel('Nucleus size (um^2)',fontsize=26)
    #plt.title("Nucleus size for micropatterned versus cultured")
...
def compare_cell_area(file_handler, sec_file_handler, acquisition1, acquisition2):
    ...
    #print(cell_area_arhgdia_cultured)
    fig = plt.figure(figsize=(15, 15))
    #plt.boxplot([cell_area_arhgdia, cell_area_arhgdia_cultured])
    ax = fig.add_subplot(111)
    ax.yaxis.grid(which="major", color='black', linestyle='-', linewidth=0.25)
    ax.tick_params(right=False, top=False, direction='inout', length=8, width=3, colors='black')
    ...
    plt.yticks(fontsize=50)
    plt.xticks(fontsize=26)
    #plt.ylabel('Cell size (um^2)',fontsize=26)
    #plt.title("Cell size for micropatterned versus standard",fontsize=20)
...
def compare_spots_density(file_handler, sec_file_handler, acquisition1, acquisition2):
    ...
    #ax1.set_title('Micropatterned cells',fontsize=20)

    #ax2.scatter(xs2, ys2, c='#ff0000')
    #ax2.scatter(xs2, ys2)
    ...
    #ax2.set_title('Standard culture cells',fontsize=20)

    ax2.plot(xs2, ys2, 'yo', xs2, fit_fn2(xs2), '--k',c='#646464')

    #ax1.set_xlabel('Cell size (um^2)',fontsize=22)
    #ax1.set_ylabel('Transcript number')
    ax2.axis([200, 1500, -100, 600])
    ax1.axis([200, 800, -100, 600])


    #ax2.set_xlabel('Cell size (um^2)',fontsize=22)
    #ax2.set_ylabel('Transcript number',fontsize=22)
...
def box_plot_fraction_profile(fractions1,fractions2,fraction1,fraction2,genes):
    ...
    bplot2 = axes[1].boxplot(fractions2,
                             ...
                             patch_artist=True)  # fill with color
    # fill with colors
    #colors = ['lightblue', 'blue', 'lightgreen', 'orange', 'red', 'yellow']
