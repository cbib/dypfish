...
import acquisition_descriptors as adsc
...
def bar_profile_median(median,genes,molecule_type, figname, err):
    ...
    #ax.set_ylim(0, ylim + 2 * ystd)
    ax.set_ylim(0, 15)

    #ax.set_ylabel('Degree of clustering(log)')
    #ax.set_title(molecule_type + ' degree of clustering')
    ax.set_xticks([])
...
def fraction_profile(fractions,fraction,genes,figname,colors):
    ...
    rects1 = ax.bar(ind, fractionsMeans, width,
                    color=colors,
                    yerr=fractionsStd,
                    error_kw=dict(elinewidth=1, ecolor='black'))
    # axes and labels
...
def histogram_noise_measured(nm_arhgdia,nm_arhgdia_cultured):
    ...
    ## the bars
    rects1 = ax.bar(ind, [nm_arhgdia_cultured, nm_arhgdia], width,
                    color=['grey', 'blue'])
    # axes and labels
    ...
    ax.set_title('')
    # xTickMarks = ["Standard cells","Micropatterned cells"]
    xTickMarks = ["", ""]

    ax.set_xticks(ind)
    xtickNames = ax.set_xticklabels(xTickMarks, ha='center')
...
def profile(profiles, genes, slice_number, figname,figtitle,colors,save=False):
    #cols = ['black', 'blue', 'green', 'orange', 'red', 'yellow', 'grey']
    #colors =['#0A3950','#1E95BB','#A1BA6D','#F16C1B','#C02A18','#E9CB45']
    ...
    #plt.xlabel('spots z-lines distance (pixels)')
    #plt.xlabel('Periphery: cytoplasm width ratio (%)')
    #plt.title(figtitle)
    ...
    ## the data
    #print(np.around(np.amax(profiles)))
    #plt.axis([0, slice_number - 1, 0, np.around(np.amax(profiles)) + 0.5])
...
def dynamic_profiles(mrna_data,protein_data,gene,plot_color,xlabel,ylabel,figpath):
    ...
    solid_mrna, = plt.plot(x_mrna, m_y_new, linestyle="-", color=plot_color, linewidth=2)
    ...
    dashed_protein, = plt.plot(x_protein, p_y_new, linestyle="--", label="Protein", color=plot_color,
                               linewidth=2)
...
def boxplot(vec_a,vec_b,figpath):
    ...
    # ax.set_xticklabels(['corresponding \n pairs', 'Non corresponding \n pairs'], fontsize=12)
    # ax.set_ylabel('correlation between \n mRNA-protein profiles', fontsize=14)
    plt.savefig(figpath, format='png')
...
def process_data(file_handler, timepoints, molecule_type, gene, calc_function, *args):
    ...
    # compute median, low and up enveloppe
...
#TODO get mrna_tp and protein_tp dynamically
#TODO find a way to avoid returning i
