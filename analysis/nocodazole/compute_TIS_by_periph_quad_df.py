...
def main():
    ...
    enable_logger()
    try:
        df = pd.read_csv(path.analysis_dir + "nocodazole/dataframe/periph_global_mtoc_file_mrna.csv")
    except IOError:
        print "Couldn't load file : periph global_mtoc_file_mrna.csv"
    ...
    try:
        df = pd.read_csv(path.analysis_dir + "nocodazole/dataframe/periph_global_mtoc_file_protein.csv")
    except IOError:
        print "Couldn't load file : periph global_mtoc_file_protein.csv"
        print "Maybe MTOC analysis hasn't been launched prior to this one"
        exit(1)
    ...
    with h5py.File(path.data_dir+input_dir_name+'/'+basic_file_name,  "r") as file_handler, \
        ...
        for mrna in mrnas:
            ...
            for timepoint in mrna_timepoints:
                ...
                mrna_tp_df.to_csv(path.analysis_dir + "nocodazole/dataframe/periph_" + mrna + '_' + timepoint + "_mrna.csv")
                time_count += 1
            gene_count += 1
        gene_count = 0
        for protein in proteins:
            ...
            for timepoint in prot_timepoints:
                ...
                prot_tp_df.to_csv(path.analysis_dir+"nocodazole/dataframe/periph_"+protein + '_' + timepoint +"_protein.csv")
                time_count += 1
            gene_count += 1
