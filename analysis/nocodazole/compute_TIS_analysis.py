...
if __name__ == "__main__":
    ...
    for mrna in mrnas:
        ...
        for timepoint in mrna_timepoints:
            ...
            mrna_df = pd.read_csv(path.analysis_dir+"nocodazole/dataframe/"+mrna + '_' + timepoint + "_mrna.csv", index_col=0)
            mrna_list.append(mrna_df.median(axis=0).values)
        for timepoint in prot_timepoints:
            prot_df = pd.read_csv(path.analysis_dir+"nocodazole/dataframe/"+mrna + '_' + timepoint + "_protein.csv", index_col=0)
        ...
        ax.set_title(mrna)
        figname = check_dir(path.analysis_dir + 'nocodazole/figures/TIS/')+mrna+'_TIS_correlation_ranking.svg'
        plt.savefig(figname,format='svg')
        plt.close()
        count_gene+=1

    ylabel='Global temporal interaction score'
    figname = check_dir(path.analysis_dir + 'nocodazole/figures/TIS/')+ 'TIS.svg'
