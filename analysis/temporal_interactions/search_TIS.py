...
def compute_TIS(configData, degree_max_mrna, degree_max_protein, number_of_quad):
    ...
    with h5py.File(path.data_dir + input_dir_name + '/' + basic_file_name, "r") as file_handler, \
        ...
        #                     value = (int(slice_area) * number_of_quad) + int(quad - 1)
        ...
        for protein in proteins:
            ...
            for timepoint in timepoints_protein:
                ...
                image_count = 0
                h_array = np.zeros((len(degree_max_protein[key]), number_of_quad * stripe_n))
                for i in range(len(degree_max_protein[key])):
                    ...
                    for i in range(1, 99):
                        for j in range(1, number_of_quad+1):
                            ...
                            value = (int(value) * number_of_quad) + int(j - 1)
...
def main():
    ...
    if (is_periph):
        try:
            ...
            print "Couldn't load file : ", path.analysis_dir + 'MTOC/dataframe/' + 'periph_global_mtoc_file_mrna.csv'
        ...
        try:
            df = pd.read_csv(path.analysis_dir + 'MTOC/dataframe/' + 'periph_global_mtoc_file_protein.csv')
        except IOError:
            print "Couldn't load file : ", path.analysis_dir + 'MTOC/dataframe/' + 'periph_global_mtoc_file_protein.csv'
            print "Maybe MTOC analysis hasn't been launched prior to this one"
            exit(1)
