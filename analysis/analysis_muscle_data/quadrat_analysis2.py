...
def show_descriptors(cell_mask,spots,nucleus_mask):
    ...
    for n, contour in enumerate(contours):
        ##print(contour[:, 1]-50)
        plt.plot(contour[:, 1]-50, contour[:, 0], color='red', linewidth=2)
...
if __name__ == "__main__":
    ...
    with h5py.File(basic_file_path, "a") as file_handler, h5py.File(muscle_rebuild_file_path, "a") as muscle_file_handler:
        ...
        for gene in genes:
            ...
            plt.savefig("quadrat_analysis_"+gene+"_"+timepoint+".png")
            plt.close()

        figname="clustering_index.png"
        stan.plot_bar_profile(vmr_values_sig, genes, 1.0, 'index of clustering', figname,colors)
