    ...
    with h5py.File(basic_file_path, "a") as file_handler, h5py.File(muscle_rebuild_file_path, "a") as muscle_file_handler:
        ...
        for gene in genes:
            ...
            for im in muscle_file_handler[molecule_type[0] + '/' + gene + '/' + timepoint]:
                ...
                plt.ylim((0,0.4))
                image_cpt+=1
                plt.savefig(check_dir(path.analysis_dir+"/analysis_muscle_data/heatmap/"+"bucket_"+ str(int(band_n)))+"/mrna_distribution_" + gene + "_" + timepoint +"_image_"+str(image_cpt)+ ".png")
                plt.close()
