...
def cytoplasmic_spread(file_handler, genes, path_data, plot_colors,  image_width, image_height,molecule_type):
    cyt_spreads = []
    ...
    for gene in tqdm.tqdm(genes, desc="Processing mRNAs"):
        ...
        cyt_spreads.append(adsc.compute_cytoplasmic_spread_2D(file_handler, image_list, image_width, image_height,))
...
def main():
    ...
    with h5py.File(path.data_dir+input_dir_name+"/"+basic_file_name, "r") as file_handler:
        cytoplasmic_spread(file_handler, genes, path.path_data, plot_colors_cytoD, image_width, image_height, molecule_type='mrna')
        cytoplasmic_spread(file_handler, genes, path.path_data, plot_colors_cytoD, image_width, image_height, molecule_type='protein')
