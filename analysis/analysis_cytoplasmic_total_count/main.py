...
def cytoplasmic_total_dynamic_profiles(file_handler,genes,proteins):
    ...
    for mrna_data, protein_data, i in data_generator:
        figpath = check_dir(path.analysis_dir + 'analysis_cytoplasmic_total_count/figures/')+'cyt_total_' + genes[i] + ".png"
        plot.dynamic_profiles(mrna_data, protein_data, genes[i], plot_colors[i], 'Time(hrs)', 'Cytoplasmic total',figpath)

def cytoplasmic_total_count(file_handler,molecule_type,genes,y_lim):
    cytoplasmic_total = []
    for gene in genes:
