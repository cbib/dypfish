    ...
    with h5py.File(path.basic_file_path, "a") as file_handler, \
        ...
        for j in range(500):
            ...
            for i in range (1,40):
                ...
                #mse = mean_squared_error(mean_arr, peripheral_profiles[randint(0, peripheral_profiles.shape[0] - 1),:])
                #mse = mean_absolute_error(mean_arr, peripheral_profiles[randint(0, peripheral_profiles.shape[0] - 1),:])
                #mse = mean_absolute_deviation(arr[rand_idx,:])
        ...
        for j in range(len(spots_peripheral_distances)):
            for i in range(0, 10):
                #peripheral_profiles[j, i] = float(len(np.where(np.array(spots_peripheral_distances[j]) <= (i + 1))[0])) / float(len(spots_peripheral_distances[j]))
                # peripheral_profiles[j,i]=float(len(np.where(spots_peripheral_distances[j] <= (i + 1))[0]))/float(len(spots_peripheral_distances[j]))
                peripheral_profiles[j, i] = float(len(np.where((spots_peripheral_distances[j] >= ((i * 10) + 1)) & (spots_peripheral_distances[j] <= (i + 1) * 10))[0]) / float(len(spots_peripheral_distances[j])))
        ...
        for j in range(500):
            ...
            for i in range(1, 24):
                ...
                #mse = mean_absolute_error(mean_arr, peripheral_profiles[randint(0, peripheral_profiles.shape[0] - 1),:])
                #mse = mean_absolute_deviation(arr[rand_idx, :])
                #mse = median_absolute_deviation(arr[rand_idx, :])
                arr_diff = mean_arr - peripheral_profiles[rand_idx, :]
                mse = mean_absolute_deviation(arr_diff)
                mads.append(mse)
